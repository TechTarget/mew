...
var handlerMap = {
  ...
  // check that the environment specified is in the available environments array
  correctEnvironment: function(args) {
    if (!_.contains(release.environment.names, args[0])) {
      throw msg.con('error', 'invalidEnvSpecified');
    }
  },

  // check that the asset(s) specified are valid
  correctAssets: function(args) {
    if (!build.isValidAsset(args[0])) {
      throw msg.con('error', 'invalidAssetTypeSpecified');
    }
  },
